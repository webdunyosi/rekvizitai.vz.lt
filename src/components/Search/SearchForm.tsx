...
export function SearchForm({
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    const searchParams = filterEmptyParams({
      ...
      city: values.city || "",
      sic_code: values.scope?.length ? values.scope : undefined,
      page: currentPage,
      page_size: 10,
    });
        ...
        >
          ...
          <div className="flex items-center space-x-2 self-end pt-2">
            <Button
              variant="secondary"
              type="submit"
              className="flex w-32 items-center bg-main-blue text-white hover:bg-black"
              disabled={isLoading}
            >
              {isLoading ? <Loader2 className="mr-2 animate-spin" /> : "Search"}
            </Button>
            <Link href={"/detailed-search"} className="text-main-blue">
              Detailed search
            </Link>
          </div>
        </form>
